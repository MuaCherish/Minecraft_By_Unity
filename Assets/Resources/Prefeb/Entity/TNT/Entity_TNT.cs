...
public class Entity_TNT : MonoBehaviour, IEntityBrain
{
    ...
    public void OnStartEntity(Vector3 _pos, bool _ActByTNT)
    {
        ...
        managerhub.Service_Music.PlayOneShot(MusicData.TNT_Fuse);
    ...
    public void OnEndEntity()
    {
        ...
        //玩家扣血
        if (MC_Runtime_DynamicData.instance.GetGameMode() == GameMode.Survival && _Direction.magnitude <= 4)
            managerhub.lifeManager.UpdatePlayerBlood((int)Mathf.Lerp(30, 10, _value), true, true);

        //Music
        managerhub.Service_Music.PlayOneShot(MusicData.explore);
        ...
        //搜索范围内所有实体
        if (managerhub.Service_Entity.GetOverlapSphereEntity(_center, TNT_explore_Radius + 2f, GetComponent<MC_Component_Registration>().Component_Registration_GetEntityId()._id, out List<EntityInfo> _entities))
        {
