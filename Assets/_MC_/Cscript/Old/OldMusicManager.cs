...
public class OldMusicManager : MonoBehaviour
{
        ...
        if (MC_Runtime_DynamicData.instance.GetGameState() == Game_State.Playing)
        {
            ...
            if (!isbroking)
            {
                ...
                if (Input.GetMouseButton(0))
                {
                    ...
                    MC_RayCastStruct _rayCast = MC_Static_Raycast.RayCast(managerhub, MC_RayCast_FindType.OnlyFindBlock,player.cam.position, player.cam.transform.forward, player.reach, -1, 0.1f);
    ...
    void ifmovingStateSwitch()
    {
        ...
        {
            managerhub.Service_Music.PlayOneShot(MusicData.fall_water);
